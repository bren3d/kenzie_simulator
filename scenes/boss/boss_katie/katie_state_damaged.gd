...
func enter() -> void:
	...
	katie.damaged_audio_player.stream = scream_audio_streams.pick_random()
	katie.damaged_audio_player.play()
	current_delay_sec = 0.0
	katie.kill_box.set_monitoring.call_deferred(false)
	

func exit() -> void:
	katie.is_damagable = true
	katie.katie.show.call_deferred()
	katie.kill_box.set_monitoring.call_deferred(true)

func update_process(delta: float) -> void:
	current_delay_sec += delta
	if current_delay_sec > charge_delay:
		transition_requested.emit(&"dead" if katie.current_hit_count >= katie.max_hp else &"charge")
