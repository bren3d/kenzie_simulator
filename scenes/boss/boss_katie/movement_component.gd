...
@export var ignore_angle: bool = false

@export_range(0.0, 720.0, 10.0, "suffix:°/s", "radians_as_degrees") 
...
@export var target_position: Vector3: set = set_target_position
...
func _physics_process(delta: float) -> void:
	...
	var target_y_angle: float = atan2(dir.x, dir.z) # TODO: account for being upside down
...
func interpolate_angle(target_y_angle: float, delta: float) -> void:
	if not turn_speed_rads_sec:
		host.global_rotation.y = target_y_angle
	else:
...
func set_target_position(val: Vector3) -> void:
	target_position = val
	if not is_node_ready() and not active: return
	active = true
	started.emit()
